(defproject lecture4 "0.1.0-SNAPSHOT"
  :description "lecture 4: importing functions"
  :url "https://github.com/hongseok-yang/probprog18/tree/master/Lectures/Lecture4"
  :license {:name "EPL-2.0 OR GPL-2.0-or-later WITH Classpath-exception-2.0"
            :url "https://www.eclipse.org/legal/epl-2.0/"}
  :dependencies [[org.clojure/clojure "1.10.1"]
                 [anglican "1.1.0"]
                 [nstools "0.2.4"]
                 [anglican "1.0.0"] 
                 [org.nfrac/cljbox2d.testbed "0.5.0"] 
                 [org.nfrac/cljbox2d "0.5.0"] 
                 [org.clojure/data.priority-map "0.0.7"] 
                 [net.mikera/core.matrix "0.33.2"] 
                 [net.mikera/core.matrix.stats "0.5.0"] 
                 [net.mikera/vectorz-clj "0.29.0"]]
  :main ^:skip-aot gorilla-test.core
  :plugins [[org.clojars.benfb/lein-gorilla "0.6.0"]]
  :target-path "target/%s"
  :resource-paths ["programs"]
  :repl-options {:init-ns lecture5.core})

